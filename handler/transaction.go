...
func (h *transactionHandler) CreateTransaction(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIResponse("Failed to create transaction", 400, "error", errorMessage)
		c.JSON(400, response)
		return
	}
	currentUser := c.MustGet("currentUser").(user.User)
	...
	if err != nil {
		response := helper.APIResponse("Failed to create transaction", 500, "error", nil)
		c.JSON(500, response)
		return
	}
	response := helper.APIResponse("Transaction created", 200, "success", transaction)
	c.JSON(200, response)
}
...
func (h *transactionHandler) GetNotification(c *gin.Context) {
	...
	if err != nil {
		response := helper.APIResponse("Failed to process notification", 400, "error", nil)
		c.JSON(400, response)
		return
	}
	...
	if err != nil {
		response := helper.APIResponse("Failed to process notification", 500, "error", nil)
		c.JSON(500, response)
		return
	}
